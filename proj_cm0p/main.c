...
{
    .intrSrc = ((NvicMux3_IRQn << CY_SYSINT_INTRSRC_MUXIRQ_SHIFT) | (cy_en_intr_t)(cpuss_interrupts_ipc_0_IRQn + USED_IPC_CHANNEL)),
    .intrPriority = 2UL
};
        ...
        if(CY_IPC_DRV_SUCCESS == Cy_IPC_Drv_ReadMsgWord(Cy_IPC_Drv_GetIpcBaseAddress(USED_IPC_CHANNEL), &u32Led))
        {
            switch(u32Led)
            {
                case 0:
                    ...
                    Cy_GPIO_Write(CYBSP_USER_LED1_PORT, CYBSP_USER_LED1_PIN, HIGH);   /* Led 1 On */
                    break;
                case 1:
                    /* cyhal_gpio_write(CYBSP_USER_LED, CYBSP_LED_STATE_OFF);*/  /* Led 1 Off */
                    Cy_GPIO_Write(CYBSP_USER_LED1_PORT, CYBSP_USER_LED1_PIN, LOW);  /* Led 1 Off */
                    break;
                default:
                    break;
            }
